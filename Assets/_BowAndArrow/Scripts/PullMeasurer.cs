...
public class PullMeasurer : UnityEngine.XR.Interaction.Toolkit.Interactables.XRBaseInteractable
{
    ...
    public void ResetPull()//new
    {
        PullAmount = 0f;
    }
    protected override void OnSelectEntered(SelectEnterEventArgs args)//new
    {
        base.OnSelectEntered(args);

        // Prevent pull if no arrow is on the notch
        if (notch == null || !notch.hasSelection)
        {
            // Cancel selection
